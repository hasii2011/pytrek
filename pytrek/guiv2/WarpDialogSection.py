...
class WarpDialogSection(BaseSection):
    ...
    def __init__(self, left: int, bottom: int, width: int, height: int, completeCallback: DialogCallback, **kwargs):
        ...
        warpFactorLayout:    UIBoxLayout = self._createWarpFactorInputBox()
        quadrantInputLayout: UIBoxLayout = self._createInputQuadrant()
        buttonLayout:        UIBoxLayout = self._createDialogButtons()

        self._vBox.add(warpFactorLayout.with_space_around(bottom=20))
        self._vBox.add(quadrantInputLayout.with_space_around(bottom=10))
        self._vBox.add(buttonLayout)
    ...
    def _createWarpFactorInputBox(self) -> UIBoxLayout:
        ...
        layout:    UIBoxLayout = UIBoxLayout(vertical=False, align='x:right, y:center')
        warpLabel: UILabel     = self._createLabel(text='Warp Factor: ')
        warpInput: UIInputText = UIInputText(text='5', height=18, width=50, font_size=12, text_color=BLACK)

        layout.add(warpLabel.with_space_around(left=5, top=5))
        layout.add(warpInput
                   .with_space_around(left=5, right=5, top=5)
    ...
    def _createInputQuadrant(self) -> UIBoxLayout:
        ...
        layout: UIBoxLayout = UIBoxLayout(vertical=False)

        xCoordinate: UIInputText = UIInputText(text='0', height=18, width=50, font_size=12, text_color=BLACK)
        yCoordinate: UIInputText = UIInputText(text='0', height=18, width=50, font_size=12, text_color=BLACK)
    ...
    def __validateWarpFactor(self, warpFactor: str) -> bool:
        ...
        try:
            ...
            if floatWarp < 0 or floatWarp > 9.9:
                self._displayError(WarpDialogSection.WARP_FACTOR_ERROR_MESSAGE)
    ...
    def __validateCoordinate(self, coordinate: str):
        ...
        try:
            ...
            if intCoordinate < 0 or intCoordinate > 9:
                self._displayError(WarpDialogSection.COORDINATES_ERROR_MESSAGE)
    ...
    def _displayError(self, msg: str):
        ...
        StdMsgBox.displayMessageBox(uiManager=self.manager, msg=msg)
